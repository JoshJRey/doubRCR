...
## chunks as specified by n.cores. Each split is run over a separate core, to allow for parallel processing. Each batch needs to be submitted as a separate task on the cluster computing, so you'll have several batches running 
## at once to speed things up. If not wanted, just set batch to 1. Make sure splits and n.cores are the same.
...
# Just remove the ones with zero length. Comment out if these are of interest for debugging. Most of these will be duplicates that arise when running in parallel.
