...
### This is a modified version of tri_rate.py that instead returns the counts for each double-singleton,
### and the reference base counts, to allow calculation of the probabilities externally
...
### Access genotype data, and reference and alternative bases
...
ds_name = "/rds/general/user/jjr18/results/" + str(type) + "/ds_counts_"+ str(sim) +".txt"
# Save ds_counts to a file, each value in a column
np.savetxt(ds_name, ds_counts[np.newaxis], fmt='%d')

ref_name = "/rds/general/user/jjr18/results/" + str(type) + "/ref_counts_"+ str(sim) +".txt"
# Save ref_counts to a file, each value in a column
np.savetxt(ref_name, ref_counts[np.newaxis], fmt='%d')
